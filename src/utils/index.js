 ...
 * 手机号验证
 * 已注入所有Vue实例，
 * template模板里调用 $checkPhone
 * 组件方法里调用 this.$checkPhone
 * 参数 phone 
 ...
 * 消息提示框
 * 已注入所有Vue实例，
 * template模板里调用 $msgOpen
 * 组件方法里调用 this.$msgOpen
 * 例：this.$msgOpen(this,'标题','这里是内容消息',function(val){}) 
 */
const msgOpen = (_this, title, content, _callback) => {
 ...
 * 获取数组最大值
 * 已注入所有Vue实例，
 * template模板里调用 $objMaxVal
 * 组件方法里调用 this.$objMaxVal
 * 例：this.$objMaxVal(obj) 得到 min val
 * 参数 json object
 */
        ...
        if (o) {
            o.forEach((ele, i) => {
                if (typeof (ele.children) == "object") {
                    maxVal = parseInt(ele.id) > parseInt(maxVal) ? ele.id : maxVal;
 ...
 * template模板里调用 $trim
 * 组件方法里调用 this.$trim
 * 例：this.$trim(obj) 得到 string
 * 参数 string
 */

const trim = (str) => {
    let ss = str.replace(/(^\s*)|(\s*$)/g, "");
    return ss;
}
 ...
 * 数组 删除指定元素
 * 已注入所有Vue实例，
 * template模板里调用 $arrayRemove
 * 组件方法里调用 this.$arrayRemove
 * 例：this.$arrayRemove(arr,val) 得到 arr
 * 参数 array,obj
 */
...
/**
 * 不规则数组 判断某个属性值是否等于如入参
 * 已注入所有Vue实例，
 * template模板里调用 $arrayAttrcheck
 * 组件方法里调用 this.$arrayAttrcheck
 * 例：this.$arrayAttrcheck(arr,opts) 得到 bool
 * 参数 obj,opts
 *  opts = {
 *     attrKey:'organId',
 *     attrVal:'1748'
 *  };
 */
const arrayAttrcheck = (o, opt) => {
    let isHave = false;
    if (!o) return isHave;
    let initExec = function (obj, opts) {
        for (let a in obj) {
            if (typeof (obj[a]) == "object" && !isHave) {
                initExec(obj[a], opts);
            } else if (opts && !isHave) {
                if ((opts.attrKey == a) && (Number(opts.attrVal) == Number(obj[a]))) {
                    isHave = true;
                    break;
                }
            }
...
Vue.prototype.$msgOpen = msgOpen
Vue.prototype.$objMaxVal = objMaxVal
Vue.prototype.$trim = trim
Vue.prototype.$arrayRemove = arrayRemove
Vue.prototype.$arrayAttrcheck = arrayAttrcheck
