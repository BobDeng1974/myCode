...
-->
<template>
    <div id="index" class="main-body modInput" ref="ytzaIndex">
        <div class="main-top-left">
            <div class="left">
                <span style="color: #fff">当前所在：</span>宏观运行分析
            </div>
            <div class="right">
                <div style="float: left;font-size:12px;height:30px;line-height:30px;"> {{ currentDate }}</div>
                <!-- <div style="float: right;padding-right:30px;position: relative;">
                    <img v-if="isFull" src="../../../../static/img/fd.png" @click="fullScreen();isFull = false;" style="width:10px;position: absolute;top:10px;right: 10px;"
                    />
                    <img v-if="!isFull" src="../../../../static/img/sx.png" @click="exitFullScreen();isFull = true;" style="width:10px;position: absolute;top:10px;right: 10px;"
                    />
                </div> -->
            </div>
        </div>
        <div class="main-top-right">
            <div id="sDate" class="left float-left">
                ...
                &nbsp;&nbsp;时间：
                <el-date-picker v-model="seleDate" type="month" size='small' :clearable="false" placeholder="选择月" style="width:90px;padding:0;height:25px;"></el-date-picker>
            ...
            <div class="center float-left">
                <div class="echartsMap">
                    ...
                    <div v-if="popoverConfig.isShowRealTime" class="popover-main" :style="{left:popoverConfig.x_RealTime+'px',top:popoverConfig.y_RealTime+'px'}">
                        ...
                        <div class="center">
                            <div class="row-title">
                                <div :style="{width:'185px'}" class="column" style="text-align: left;">
                                    &nbsp;&nbsp;企业名称
                                </div>
                                ...
                                <div :style="{width:'65px'}" class="column">
                                    车辆总数
                                </div>
                            ...
                            <div style="clear: both;"></div>
                            <div v-for="(item,key,index) in arearOrganSummaryObj.resData.data" :key="index" :style="{background:'#233D52'}" class="row-data">
                                <div v-bind:title="item.OrgName" :style="{width:'185px'}" style="text-align:left;" class="column1">
                                    &nbsp;&nbsp;{{ item.OrgName | textFormat(11,true) }}
                                </div>
                                <div :style="{width:'65px'}" class="column1">
                                    {{ item.TruckCount }}
                                </div>
...
<script>
    import echartsYearassessment from "@/components/child/dydp/EchartsYearAssessment.vue";
    import echartsMonthassessment from "@/components/child/dydp/EchartsMonthAssessment.vue";
    import echartsPeccancyproportion from "@/components/child/dydp/EchartsPeccancyProportion.vue";
    import echartsBarregionalrank from "@/components/child/dydp/EchartsBarRegionalRank.vue";
    import echartsMap from "@/components/child/dydp/EchartsMap.vue";
    import echartsChinamap from "@/components/child/dydp/EchartsChinaMap.vue";
    ...
    export default {
        ...
        methods: {
            ...
            searchData() {
                ...
                //年度考核指标
                _this.yearAssessmentObj.param.years =
                    sYear - 2 + "," + (sYear - 1) + "," + sYear; //years: '2016,2017,2018', //年份，逗号分隔
                _this.ajaxUtilFun(2, _this.yearAssessmentObj.param);
                //月度考核指标
                let lastY = sYear - 1 + sMonth;
                let lastM =
                    _this.seleDate.getMonth() == 0 ?
                    sYear - 1 + "12" :
                    sYear +
                    (_this.seleDate.getMonth().toString().length > 1 ?
                        _this.seleDate.getMonth().toString() :
                        "0" + _this.seleDate.getMonth().toString());
                _this.monthAssessmentObj.param.month =
                    lastY + "," + sYear.toString() + sMonth + "," + lastM; //month: '201703,201803,201802' //月份，逗号分隔
            ...
            ajaxUtilFun(tId, param) {
                let _this = this;
                let aUrl = _this.global_.apiUrlDy + "";
            ...
            getYearAssessmentIndex(res) {
                ...
                for (let i = 0; i < yArr.length; i++) {
                    ...
                    for (let j = 0; j < resData.length; j++) {
                        if (yArr[i] === resData[j].Category) {
                            //日期相同
                            switch (resData[j].Name) {
                                case "入网率":
                                    _this.yearAssessmentObj.resData.series_data[
                                        i
                                    ].data[0] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                                case "遮挡报警率":
                                    _this.yearAssessmentObj.resData.series_data[
                                        i
                                    ].data[1] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                                case "百公里综合扣分":
                                    _this.yearAssessmentObj.resData.series_data[
                                        i
                                    ].data[2] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                                case "预报警处置正常率":
                                    _this.yearAssessmentObj.resData.series_data[
                                        i
                                    ].data[3] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                                case "设备正常运行率":
                                    _this.yearAssessmentObj.resData.series_data[
                                        i
                                    ].data[4] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                            }
                        }
                    }
            ...
            getMonthAssessmentIndex(res) {
                ...
                for (let i = 0; i < mArr.length; i++) {
                    this.monthAssessmentObj.resData.series_data[i].name =
                        mArr[i].substring(0, 4) + "年" + mArr[i].substring(4, 6) + "月"; //赋日期
                    for (let j = 0; j < resData.length; j++) {
                        if (mArr[i] === resData[j].Category) {
                            //日期相同
                            switch (resData[j].Name) {
                                case "百公里综合扣分":
                                    _this.monthAssessmentObj.resData.series_data[
                                        i
                                    ].data[0] = parseFloat(resData[j].Value).toFixed(2);
                                    break;
                                case "遮挡报警率":
            ...
            getPeccancyProportion(res) {
                ...
                resData.PeccancyProportion.forEach(function (val, i) {
                    switch (resData.PeccancyProportion[i].name) {
                        case "班线客车":
                            _this.peccancyProportionObj.resData.series_data[0] =
                                resData.PeccancyProportion[i].record;
                            break;
                        case "旅游客车":
                            _this.peccancyProportionObj.resData.series_data[1] =
                                resData.PeccancyProportion[i].record;
                            break;
                        case "普通货车":
                            _this.peccancyProportionObj.resData.series_data[2] =
                                resData.PeccancyProportion[i].record;
                            break;
                        case "危险品车":
                            _this.peccancyProportionObj.resData.series_data[3] =
                                resData.PeccancyProportion[i].record;
                            break;
                    }
                });
            ...
            getRegionalComprehensiveRank(res) {
                ...
                if (resData.length > 0) {
                    ...
                    _this.regionalRankObj.resData.series_data = [
                        resData[0].Comprehensive,
                        resData[1].Comprehensive,
...
<style >
    ...
    .main-top-left {
        ...
        padding: 0 10px;
        color: #44a8fa;
        filter: "progid:DXImageTransform.Microsoft.gradient (GradientType=0, startColorstr=#416386, endColorstr=#345c86)";
        /*IE<9>*/
        -ms-filter: "progid:DXImageTransform.Microsoft.gradient (GradientType=0, startColorstr=#416386, endColorstr=#345c86)";
        /*IE8+*/
    ...
    .main-top-right {
        ...
        color: #fff;
        border: 0;
        filter: "progid:DXImageTransform.Microsoft.gradient (GradientType=0, startColorstr=#25374d, endColorstr=#2c4563)";
        /*IE<9>*/
        -ms-filter: "progid:DXImageTransform.Microsoft.gradient (GradientType=0, startColorstr=#25374d, endColorstr=#2c4563)";
        /*IE8+*/
        background: -webkit-linear-gradient( top,
        rgb(37, 55, 77),
        rgb(44, 69, 99),
    ...
    .modInput .el-pagination__total {
        margin-right: 10px;
        font-weight: 400;
        color: #66bcff;
    }

    .modInput .el-pagination button:hover {
        color: #fff;
    }

    .modInput .el-pagination .btn-next,
    ...
    .modInput .el-pager li.active {
        color: #fff !important;
        cursor: default !important;
    }

    .modInput .el-pagination button:disabled {
...
/* 
    ...
    body .el-popper[x-placement^="bottom"] .popper__arrow {
        ...
        border-bottom-color: #111930 !important;
    }
    ...
    body .el-date-picker .el-picker-panel__content {
        width: auto !important;
    }
    ...
    .modInput .el-input--suffix .el-input__inner {
        padding-right: 0 !important;
    }

    .modInput .el-input__prefix {
        display: none;
    } */
