...
(function() {
     ...
     */
     BMapLib.LuShu = function(map, path, opts,pSpeed) {
...
            } 
     ...
     * @return 无返回值.
     */
    LuShu.prototype.pause = function(speed) {
		let me = this;
		if(speed){
			me._opts.speed = speed;
		} 
	//	console.log(me._opts.speed);
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _addMarker: function(callback) {
            ...
            this._marker = marker;
			this._opts.markerLabel && this._addMarkerLabel(0);
        },
        /**
         * 添加上方overlay
         * @return 无返回值.
		...
		_addMarkerLabel: function(index) {
			let me = this;
			if(!me._opts.markerLabel.isShow) return;
			if(!me._label){
				let label = new BMap.Label(
                    "<div style='width:30px;height:30px;text-align:center;background-color: rgba(0,0,0,0);;font-weight: bold;;border-radius:10px;'>"
                        +"<div style='font-size:12px;width:100%;height:12px;text-align:center;'>"+
						    me._pSpeed[0].speed
                        +"</div>"
                    +"<div style='width:100%;font-size:10px;;text-align:center;lineHeight:10px;'>km/h</div>  </div>", {
						offset: new BMap.Size(me._opts.markerLabel.offsetSize.x,me._opts.markerLabel.offsetSize.y)
					});
				label.setStyle({
					backgroundColor: " rgba(0,0,0,0) ",
					border: "0",
					display: me._opts.markerLabel.isShow?'block':'none',
					fontWeight: "bold"
				});
				me._marker.setLabel(label);
				me._label = label;
			}else{
				me._label.setStyle({display:me._opts.markerLabel.isShow?'block':'none'});
				me._label.setContent(
                        "<div style='width:30px;height:30px;text-align:center;background-color: rgba(0,0,0,0);;font-weight: bold;;border-radius:10px;'>" 
                        + "<div style='font-size:12px;width:100%;height:12px;text-align:center;'>"+
						    me._pSpeed[index].speed
                        +"</div>"
                        +"<div style='width:100%;font-size:10px;;text-align:center;lineHeight:10px;'>km/h</div>  </div>");
			}
			 
		},
		_addPolyLine:function(potArr){
			let me = this;
			if(!potArr || !me._opts.polyLine.isShow)return;
			//显示折线 
			me._map.addOverlay(new BMap.Polyline(potArr, { 
				strokeColor : me._opts.polyLine.color, 
				strokeWeight : me._opts.polyLine.weight, 
				strokeOpacity : me._opts.polyLine.opacity 
			})); // 画线 
		},
        /**
         * 获取墨卡托坐标
        ...
        _move: function(initPos,targetPos,effect) {
            ...
            var me = this,
				...
				me._opts.polyLine.isShow && me._addPolyLine(pointsArr);
        ...
        */
        ...
        _moveNext: function(index) {
            var me = this;
			me._opts.markerLabel.isShow && me._addMarkerLabel(index);
